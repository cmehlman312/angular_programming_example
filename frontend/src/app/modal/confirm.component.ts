...
import {
  ...
  ModalSize,
} from '@giomamaladze/ng2-semantic-ui';
...
@Component({
  ...
  template: `
    <div class="header" *ngIf="modal.context.title">
      {{ modal.context.title }}
    </div>
    <div class="content">
    ...
    <div class="actions">
      <button class="ui red button" (click)="modal.deny(undefined)">
        {{ modal.context.cancelButton || 'Cancel' }}
      </button>
      <button
        ...
        autofocus
      >
        {{ modal.context.confirmButton || 'OK' }}
      </button>
    </div>
  `,
})
